...
class DeepSearchAgent:
    ...
    def research(self, query: str, save_report: bool = True) -> str:
        ...
        if self.config.FAST_TEST_MODE:
            logger.warning("[FAST TEST] MediaEngine 进入快速测试模式")
            ...
            if self.config.TEST_SEARCH_AND_ANALYSIS:
                ...
                try:
                    # 尝试执行一次综合搜索，只取1条结果
                    test_response = self.execute_search_tool("comprehensive_search", query, max_results=1)
                    if test_response and test_response.webpages:
...
"""
            ...
            self.state.report_title = "MediaEngine Test Report"
