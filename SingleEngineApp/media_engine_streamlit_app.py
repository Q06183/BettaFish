...
def main():
    ...
    logger.info(f"Media Engine configuration reloaded: FAST_TEST_MODE={settings.FAST_TEST_MODE}, TEST_SEARCH_AND_ANALYSIS={settings.TEST_SEARCH_AND_ANALYSIS}")
...
def execute_research(query: str, config: Settings):
    ...
    try:
        ...
        if config.FAST_TEST_MODE:
            logger.warning("[FAST TEST] MediaEngine (Streamlit) 进入快速测试模式")
            ...
            if config.TEST_SEARCH_AND_ANALYSIS:
                ...
                try:
                    # 尝试执行一次综合搜索，只取1条结果
                    test_response = agent.execute_search_tool("comprehensive_search", query, max_results=1)
                    # 兼容不同agent的返回格式
                    if hasattr(test_response, 'webpages') and test_response.webpages:
                        search_verification = f"✅ 搜索工具验证成功 (找到 {len(test_response.webpages)} 条结果)"
                        logger.info(search_verification)
                    elif hasattr(test_response, 'results') and test_response.results: # Anspire可能返回results
            ...
            mock_report = f"""# MediaEngine 测试报告
            
这是一个来自 MediaEngine 的模拟测试响应。
当前处于 FAST_TEST_MODE。
...
"""
            ...
            agent.state.report_title = "MediaEngine Test Report"
