...
class DeepSearchAgent:
    ...
    def research(self, query: str, save_report: bool = True) -> str:
        ...
        if self.config.FAST_TEST_MODE:
            logger.warning("[FAST TEST] InsightEngine 进入快速测试模式")
            
            search_verification = "未执行真实搜索验证"
            if self.config.TEST_SEARCH_AND_ANALYSIS:
                logger.info("[FAST TEST] 正在验证搜索工具连通性...")
                try:
                    # 尝试执行一次热点内容搜索（通常最快且不需要复杂参数）
                    test_response = self.execute_search_tool("search_hot_content", query, limit=1, enable_sentiment=False)
